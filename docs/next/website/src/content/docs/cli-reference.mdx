...
herdr tab move-to-space <tab_id> --space <workspace_id> [--focus|--no-focus]
...
`tab move-to-space` moves the whole tab, with every pane in it, and is the same operation as dragging a tab onto a space row or onto `new` in the sidebar. `--new-space` promotes it into a space of its own, placed directly after its source space, or after that space's whole worktree group, and named after the tab unless `--label` says otherwise. A space's only tab cannot move, because that would leave the space empty: the call returns `.result.move_result.changed` as `false` with `reason` `only_tab`. Moved panes keep their terminals and agents but take new workspace-qualified pane IDs; the previous IDs keep resolving as aliases.
