...
A new workspace is inserted directly after the source workspace and takes the
tab's own name when no `label` is given. It never joins the source workspace's
