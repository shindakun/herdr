...
`tab.move` reorders a tab inside its own workspace. `tab.move_to_workspace`
moves the whole tab, with every pane in it, into another workspace or into a new
one:

```json
{"id":"req_tab_to_new_space","method":"tab.move_to_workspace","params":{"tab_id":"w1:t2","destination":{"type":"new_workspace","label":"logs"},"focus":true}}
{"id":"req_tab_to_space","method":"tab.move_to_workspace","params":{"tab_id":"w1:t2","destination":{"type":"workspace","workspace_id":"w2"},"focus":true}}
```

A new workspace is inserted directly after the source workspace, or after that
workspace's whole worktree group, and takes the tab's own name when no `label`
is given. It never joins the source workspace's
worktree group. Moving a workspace's only tab would leave that workspace empty,
so it returns `changed: false` with `reason: "only_tab"`; naming the workspace
the tab already belongs to returns `reason: "same_workspace"`.

The response is `type: "tab_moved_to_workspace"` with `changed`, optional
`reason`, `previous_workspace_id`, `previous_tab_id`, the moved `tab`, its
`panes`, and an optional created workspace record. Every moved pane keeps its
terminal but takes a new public pane id in the destination workspace, and the
previous ids stay usable as aliases. Subscribers see `workspace.created` when a
workspace is created, then `tab.created` for the tab at its new identity and
`tab.closed` for the identity it left behind.
